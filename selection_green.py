...
# define a cor a ser detectada (verde)
color_lower = np.array([36, 50, 50])
color_upper = np.array([86, 255, 255])
...
while True:
    ...
    if len(contours)>0:
        ...
        if radius > 5:
            ...
            cv2.putText(frame, "Verde", (int(x - radius), int(y - radius - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            print("Cor detectada: Verde")
    else:
